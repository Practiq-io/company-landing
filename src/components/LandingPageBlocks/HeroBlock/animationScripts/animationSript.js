...
const writeCode = (txt) => {
    if (codeFlag < txt.length) {
        document.getElementById("code_block-animation-txt").innerHTML += txt.charAt(codeFlag);
        codeFlag++;
...
const removeCode = (txt) => { 
    if (codeFlag === txt.length && txt.length !== 0) {
        txt = txt.replace(/</g,'&lt;')
        codeFlag = txt.length
        document.getElementById("code_block-animation-txt").innerHTML = txt = txt.slice(0, codeFlag-1);
        codeFlag--;     
...
const writeTitle = (txt) => {
    ...
    if (titleFlag < txt.length){
        document.getElementById("code_block-animation-title").innerHTML += txt.charAt(titleFlag);
        titleFlag++;
...
const removeTitle = (txt) => {
    if(titleFlag === txt.length) {
        document.getElementById("code_block-animation-title").innerHTML = txt = txt.slice(0, titleFlag-1);
        titleFlag--;
